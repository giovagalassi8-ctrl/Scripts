...
# Restricts the substitution model to C60+G+F: a profile mixture model with 60 frequency classes, gamma rate variation (+G), and empirical amino acid frequencies (+F).
# Use a comma-separated list (e.g. "C60+G+F,LG,WAG") to allow multiple models.
MODELS="C60+G+F"
...
for DIR in "${FOLDERS[@]}"; do
    ...
    # (In this example: ML refers to a Maximum Likelyhood tree; MS80 refers to the supermatrix; allgenes refers to the fact that no one filters was applied to this matrix; MM refers to the use of Mixture Models).
    OUTDIR="$DIR/01_ML_MS80_${SUFFIX}_allgenes_MM"
    mkdir -p "$OUTDIR"
    ...
    # Set the input file.
    # Change accordingly with the correct name of yours concatenated file. 
    ...
    # Copy this file into the output directory.
    cp "$MAT" "$OUTDIR/"

    # Set the final treefile name. Change as desired (in this example, PM refers to mixture models)
    PREFIX="$OUTDIR/ML_MS80_${SUFFIX}_MM"
